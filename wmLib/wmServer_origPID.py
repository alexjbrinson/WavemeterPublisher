...
class WavemeterMultiplexer(Device):
  ...
  def load_config(self, config_path):
    ...
    if not os.path.exists(config_path):print('no config file found'); return
  ...
  def run(self):
    ...
    while self.state.running:
      try:
        ...
        if len(active_channels)==0:time.sleep(Device.sleep_time)
  ...
  def run(self):
    ...
    while self.state.running:
      try:
        ...
        else: time.sleep(Device.sleep_time)
...
if __name__ == '__main__':
  ...
  config_path=config_dir/'singletConfig.json'
  config_path2=config_dir/'switcherConfig.json'
  state = AppState()
