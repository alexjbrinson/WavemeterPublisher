...
class MainGUI(QtWidgets.QMainWindow):
  ...
  def updateConfigs(self): #this is very bad at finding when the config updated, and I don't know why.TODO
    ...
    for port in self.fos_port_list:
      if self.watching[port]:
        ...
        # print(last_config_time)
        if self.last_configs[port]!=last_config_time:
          print("config updated!")
          self.last_configs[port]=last_config_time
          self.portViewers[port].updateGUIConfig()
  ...
  def update(self):
    ...
    for fos_port in self.fos_port_list:
      pViewer=self.portViewers[fos_port]
      ...
      if self.readout[fos_port]['latest_time'] is None or self.readout[fos_port]['latest_reading']==0: continue
      # self.data[fos_port]['Times']+=[float(self.readout[fos_port]["latest_time"])]
      # self.data[fos_port]['Wavelengths']+=[float(self.readout[fos_port]["latest_reading"])]
      ...
      if self.watching[fos_port]:
        # print(self.data[fos_port]['Times'][-1], self.data[fos_port]['Wavelengths'][-1])
        # self.portViewers[fos_port].addData(self.data[fos_port]['Times'][-1], self.data[fos_port]['Wavelengths'][-1])
        pViewer.addData(float(self.readout[fos_port]["latest_time"]), float(self.readout[fos_port]["latest_reading"]))
        for parm in pViewer.labelParams:
          # print(self.readout[fos_port].keys())
          val=self.readout[fos_port][f"latest_{parm}"]
          if parm=="time" and val:
            text=datetime.fromtimestamp(val).strftime(f'%y-%m-%d\n%I:%M:%S')+f'.{int((val%1)*1000):03d}'#\ndt={round(dt,3)}'
          else: text = text=str(round(val,8))
          pViewer.widgets[parm].setText(text)
      else:
        self.data[fos_port]['Times']+=[float(self.readout[fos_port]["latest_time"])]
        self.data[fos_port]['Wavelengths']+=[float(self.readout[fos_port]["latest_reading"])]
